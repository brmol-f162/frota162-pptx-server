...
app.post('/generate', async (req, res) => {
  try {
    ...
    if (!d || !d.prospect_empresa) {
      return res.status(400).json({ error: 'Campo prospect_empresa ausente' });
    }
    ...
    s1.addText(d.slide1_titulo || `${empresa}: por que agir agora?`, { x:0.35, y:0.08, w:12.6, h:0.65, fontSize:26, bold:true, color:COR.branco, fontFace:'Montserrat', valign:'middle', margin:0 });
    s1.addText(d.slide1_subtitulo || '', { x:0.35, y:0.75, w:12.6, h:0.45, fontSize:13, color:'FFD0C0', fontFace:'Montserrat', valign:'middle', margin:0 });
    ...
    const pos = [{ x:0.30, y:1.55 },{ x:6.93, y:1.55 },{ x:0.30, y:4.15 },{ x:6.93, y:4.15 }];
    cards.forEach((c, i) => {
      const p = pos[i];
      s1.addShape(pres.ShapeType.rect, { x:p.x, y:p.y, w:5.9, h:2.45, fill:{ color: COR.card }, line:{ color: COR.divisor, width:1 } });
    ...
    s1.addText(d.slide1_footer||'', { x:0.30, y:6.90, w:11.0, h:0.55, fontSize:11, color:COR.branco, fontFace:'Montserrat', valign:'middle', margin:0 });
    s1.addText('frota162.com.br', { x:11.50, y:6.90, w:1.65, h:0.55, fontSize:10, color:COR.laranja, fontFace:'Montserrat', align:'right', valign:'middle', margin:0 });
    ...
    s2.addText(`O que a Frota162 entrega para ${empresa}`, { x:0.35, y:0.10, w:12.6, h:0.60, fontSize:22, bold:true, color:COR.branco, fontFace:'Montserrat', valign:'middle', margin:0 });
    s2.addText('Solução + Retorno Financeiro', { x:0.35, y:0.70, w:12.6, h:0.32, fontSize:12, color:'AAAAAA', fontFace:'Montserrat', valign:'middle', margin:0 });
    ...
    s2.addText(d.slide2_custo_atual||'', { x:0.45, y:1.68, w:4.40, h:1.15, fontSize:11, color:COR.dark, fontFace:'Montserrat', valign:'top', margin:0, wrap:true });
    ...
    s2.addText(d.slide2_investimento||'', { x:0.45, y:3.88, w:4.40, h:0.42, fontSize:18, bold:true, color:COR.verde, fontFace:'Montserrat', margin:0 });
    s2.addText(d.slide2_economia||'', { x:0.45, y:4.32, w:4.40, h:0.80, fontSize:11, color:COR.dark, fontFace:'Montserrat', valign:'top', margin:0, wrap:true });
    s2.addText('Pós-pago · Sem fidelidade · Aviso prévio 30 dias', { x:0.30, y:5.40, w:4.70, h:0.35, fontSize:9, color:'888888', fontFace:'Montserrat', align:'center', margin:0 });
    ...
    s2.addText(d.cta||'Solicite uma demonstração', { x:5.55, y:5.95, w:7.48, h:0.60, fontSize:14, bold:true, color:COR.branco, fontFace:'Montserrat', align:'center', valign:'middle', margin:0 });
    ...
    s2.addText('frota162.com.br', { x:11.50, y:6.90, w:1.65, h:0.55, fontSize:10, color:COR.laranja, fontFace:'Montserrat', align:'right', valign:'middle', margin:0 });
